...
fn run_init(output: PathBuf, force: bool) -> Result<()> {
    if output.exists() && !force {
        bail!("Config file {} already exists. Use --force to overwrite.", output.display());
    }
...
fn run_server(config_path: PathBuf) -> Result<()> {
    ...
    config.storage.ensure_directories().context("creating storage directories")?;
    config.database.ensure_directories().context("creating database directories")?;
